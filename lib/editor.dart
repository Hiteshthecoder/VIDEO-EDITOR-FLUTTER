...
class _EditorState extends State<Editor> {
  ...
  Future<bool> initialize() async {
    if (!isInitialized) {
      isInitialized = true;
      await FFprobeKit.getMediaInformation(editedInfo.filepath).then((session) async {
        mediaInformation = session.getMediaInformation()!;
      });
      editedInfo.frameRate = getFramerate();
      editedInfo.totalLength =
          editedInfo.end = Duration(microseconds: (double.parse(mediaInformation.getDuration()!) * 1000000).floor());
  ...
  Future<void> showInfo(String path, BuildContext context) async {
    ...
    if (streams.isNotEmpty) {
      for (var stream in streams) {
        ...
        vidInfo.add("Stream codec: ${stream.getAllProperties()!['codec_name']}");
        vidInfo.add("Stream full codec: ${stream.getAllProperties()!['codec_long_name']}");
        ...
        vidInfo.add("Stream bitrate: ${stream.getAllProperties()!['bit_rate']}");
        vidInfo.add("Stream sample rate: ${stream.getAllProperties()!['sample_rate']}");
        vidInfo.add("Stream sample format: ${stream.getAllProperties()!['sample_fmt']}");
        vidInfo.add("Stream channel layout: ${stream.getAllProperties()!['channel_layout']}");
        vidInfo.add("Stream sar: ${stream.getAllProperties()!['sample_aspect_ratio']}");
        vidInfo.add("Stream dar: ${stream.getAllProperties()!['display_aspect_ratio']}");
        vidInfo.add("Stream average frame rate: ${stream.getAllProperties()!['avg_frame_rate']}");
        vidInfo.add("Stream real frame rate: ${stream.getAllProperties()!['r_frame_rate']}");
        vidInfo.add("Stream time base: ${stream.getAllProperties()!['time_base']}");
        vidInfo.add("Stream codec time base: ${stream.getAllProperties()!['codec_time_base']}");
  ...
  double getFramerate() {
    List<String> a = (mediaInformation
            .getStreams()
            .firstWhere((element) => element.getAllProperties()!['codec_type'] == 'video')
