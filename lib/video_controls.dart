...
class VideoPlayerControlls extends StatefulWidget {
  ...
  const VideoPlayerControlls({Key? key, required this.controller, required this.framerate}) : super(key: key);
...
class _VideoPlayerControllsState extends State<VideoPlayerControlls> {
  ...
  void setPosition(bool next) {
    setState(() {
      if (next) {
        widget.controller.seekTo(widget.controller.value.position + Duration(milliseconds: 1000 ~/ widget.framerate));
      } else {
        widget.controller.seekTo(widget.controller.value.position - Duration(milliseconds: 1000 ~/ widget.framerate));
      }
    });
        ...
        Row(
          ...
          children: [
            ElevatedButton(
              onPressed: () {
                setState(() {
                  _ismuted ? widget.controller.setVolume(1) : widget.controller.setVolume(0);
                });
                _ismuted = !_ismuted;
              },
            ...
            ElevatedButton(
              onPressed: () => FullScreenView.showFullScreen(context, widget.controller),
              style: ElevatedButton.styleFrom(
                shape: const CircleBorder(),
              ),
      ...
      children: [
        ...
        FlutterSlider(
          ...
          trackBar: FlutterSliderTrackBar(
              ...
              activeTrackBar: const BoxDecoration(gradient: LinearGradient(colors: [Colors.blue, Colors.purple]))),
